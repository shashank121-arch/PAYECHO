...
import type { Contract as PayEchoContract } from '../../managed/payecho';
...
export function useMidnight() {
    ...
    const connectWallet = useCallback(async () => {
        try {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    const interval = setInterval(() => {
                        ...
                        if (attempts >= 20) { // 2 seconds max
                            clearInterval(interval);
                            reject(new Error("Wallet extension not found. Please install a Midnight compatible wallet."));
                        }
                    }, 100);
                });
            };
            ...
            // Dynamically probe wallet connectors
            const connectors = Object.values(midnight) as DAppConnectorAPI[];
            ...
            if (wallet.state && typeof wallet.state().subscribe === 'function') {
                wallet.state().subscribe((state: any) => {
                    setWalletAddress(state.address || state.publicAddress);
                ...
                throw new Error("Wallet does not support observable state.");
            }
